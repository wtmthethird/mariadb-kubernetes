...
if [ "$1" == "maxscale" ]; then
    ...
    expand_templates /mnt/config-template/maxscale/maxscale.cnf >> /etc/maxscale-cfg/maxscale.cnf
else
    ...
    if [[ ! -d /var/lib/mysql/mysql ]]; then
	  ...
	  expand_templates /mnt/config-template/mariadb-server/users.sql > /docker-entrypoint-initdb.d/init.sql
       else
	  # 
          ...
          expand_templates /mnt/config-template/mariadb-server/replication.sql >> /docker-entrypoint-initdb.d/init.sql
       fi
   fi
fi
